    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[i].length; j++) {
                ...
                int secondMatrixElement = cloneMatrix[i][j];
                if (element == secondMatrixElement) {
                    sb.append("No swap required\n");
                } else {
                    for (int p = 0; p < cloneMatrix.length; p++) {
                        for (int k = 0; k < cloneMatrix[p].length; k++) {
                            if (cloneMatrix[p][k] == element) {
                                cloneMatrix[i][j] = cloneMatrix[p][k];
                                cloneMatrix[p][k] = secondMatrixElement;
